...
const FloatingInput = ({ theme, icon: Icon, type, id, label, value, onChange, showEye, onEyeClick, isEyeOpen, ...props }: any) => (
  <div className="relative group">
    ...
    {showEye && (
      <button type="button" onClick={onEyeClick} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-700 dark:text-gray-300 focus:outline-none p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-[#1F1F2E] transition-colors z-10">
...
const BtnPrimary = ({ children, disabled, type = "submit", onClick, t, loading }: any) => (
  <button
    type={type}
    onClick={onClick}
...
const GoogleBtn = ({ handleGoogleLogin, loading }: any) => (
...
export default function LoginModal({ isOpen, onClose }: Props) {
  ...
  const googleLoginAction = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      setLoading(true);
      try {
        const data = await post("google", { accessToken: tokenResponse.access_token });
        login(data.user, data.accessToken, data.refreshToken);
        onClose();
      } catch (err: any) {
        setError(err.message);
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" onClick={e => e.target === e.currentTarget && onClose()}>
          ...
          >
            ...
            {flow === "pick-user" && (
              <>
                <ModalHeader title="User Account" back="choose" reset={reset} t={t} onClose={onClose} />
                <div className="p-6 sm:p-8 space-y-4 relative z-10 overflow-y-auto no-scrollbar">
                  <ErrorBanner error={error} />
                  <BtnPrimary type="button" disabled={false} onClick={() => reset("login")} t={t} loading={loading}>
                    <Mail className="w-4 h-4" /> Log In with Email
                  </BtnPrimary>
                  <div className="pt-1">
                    <GoogleBtn handleGoogleLogin={handleGoogleLogin} loading={loading} />
                  </div>
                  <Divider />
            ...
            {flow === "login" && (
              <>
                <ModalHeader title="Log in to your account" back="pick-user" reset={reset} t={t} onClose={onClose} />
                <div className="p-6 sm:p-8 relative z-10 overflow-y-auto no-scrollbar">
                  ...
                  <form onSubmit={handleLogin} className="space-y-4">
                    ...
                    <div className="pt-2">
                      <BtnPrimary t={t} loading={loading}>Log In</BtnPrimary>
                    </div>
                  </form>
                  
                  <Divider />
                  <p className="text-center text-[13px] text-gray-500 dark:text-gray-400 font-medium">
                    <button type="button" onClick={() => reset("pick-user")} className={`${t.linkText} font-bold hover:underline underline-offset-2`}>
                      Sign in with Google
                    </button>
                  </p>
                  <p className="text-center text-[13px] text-gray-500 dark:text-gray-400 font-medium mt-4">
            ...
            {flow === "signup-info" && (
              <>
                <ModalHeader title="Create your account" back="pick-user" reset={reset} t={t} onClose={onClose} />
                <div className="p-6 sm:p-8 overflow-y-auto no-scrollbar relative z-10">
                  ...
                  <form onSubmit={handleSignupSendOtp} className="space-y-4">
                    ...
                    <div className="pt-2">
                      <BtnPrimary t={t} loading={loading}>Send Verification OTP</BtnPrimary>
                    </div>
                  </form>
            ...
            {flow === "signup-otp" && (
              <>
                <ModalHeader title="Verify your email" back="signup-info" reset={reset} t={t} onClose={onClose} />
                <div className="p-6 sm:p-8 overflow-y-auto no-scrollbar relative z-10">
                  <div className="text-center mb-6">
                    <div className={`w-16 h-16 ${t.panelIcon} rounded-full flex items-center justify-center mx-auto mb-4`}>
                      <Mail className="w-8 h-8" />
                    </div>
                    <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 mb-1">Check your Email</h3>
                    <p className="text-[13px] text-gray-500 dark:text-gray-400 font-medium">We've sent a 6-digit code to</p>
                    <p className="text-sm font-black text-gray-800 dark:text-gray-200">{signupEmail}</p>
                  </div>
                    ...
                    <div className="flex gap-2.5 justify-center my-6">
                      {otp.map((d, i) => (
                        <input
                          ...
                          className={`w-12 h-14 sm:w-14 sm:h-16 text-center text-2xl font-black border-2 border-gray-200 dark:border-[#2A2A3A] rounded-xl bg-gray-50 dark:bg-[#151522]/50 text-gray-800 dark:text-gray-200 outline-none ${t.otpFocus} focus:bg-white dark:bg-[#0D0D17] focus:ring-4 focus:-translate-y-1 transition-all shadow-sm`}
                        />
                      ))}
                    </div>
                    <BtnPrimary disabled={otpValue.length < 6} t={t} loading={loading}>Verify & Create Account</BtnPrimary>
                  </form>
                  
                  <div className="mt-6 text-center">
                    <p className="text-[13px] text-gray-500 dark:text-gray-400 font-medium">Didn't receive the email?</p>
            ...
            {flow === "forgot-email" && (
              <>
                <ModalHeader title="Reset Password" back="login" reset={reset} t={t} onClose={onClose} />
                <div className="p-6 sm:p-8 overflow-y-auto no-scrollbar relative z-10">
                  <p className="text-[13px] text-gray-500 dark:text-gray-400 font-medium text-center mb-6">
                  ...
                  <form onSubmit={handleForgotSendOtp} className="space-y-6">
                    ...
                    <BtnPrimary t={t} loading={loading}>Send Reset Code</BtnPrimary>
                  </form>
                </div>
              </>
            )}
            ...
            {flow === "forgot-otp" && (
              <>
                <ModalHeader title="Enter Reset Code" back="forgot-email" reset={reset} t={t} onClose={onClose} />
                <div className="p-6 sm:p-8 overflow-y-auto no-scrollbar relative z-10">
                  <div className="text-center mb-6">
                    <p className="text-[13px] text-gray-500 dark:text-gray-400 font-medium">Enter the 6-digit code sent to</p>
                    <p className="text-sm font-black text-gray-800 dark:text-gray-200">{forgotEmail}</p>
                  </div>
                  <ErrorBanner error={error} />
                  <form onSubmit={handleForgotVerifyOtp}>
                    ...
                    <BtnPrimary disabled={otpValue.length < 6} t={t} loading={loading}>Verify Code</BtnPrimary>
                  </form>
                  <div className="mt-6 text-center">
            ...
            {flow === "forgot-reset" && (
              <>
                <ModalHeader title="Create New Password" back="forgot-otp" reset={reset} t={t} onClose={onClose} />
                <div className="p-6 sm:p-8 overflow-y-auto no-scrollbar relative z-10">
                  <p className="text-[13px] text-gray-500 dark:text-gray-400 font-medium text-center mb-6">Your new password must be different from previous used passwords.</p>
                  <ErrorBanner error={error} />
                  <form onSubmit={handleResetPassword} className="space-y-4">
                    ...
                    />
                    <div className="pt-2">
                      <BtnPrimary t={t} loading={loading}>Reset Password</BtnPrimary>
                    </div>
                  </form>
                </div>
              </>
            ...
            {flow === "vendor-login" && (
              <>
                <ModalHeader title="Vendor Login" back="choose" reset={reset} t={t} onClose={onClose} />
                <div className="p-6 sm:p-8 relative z-10 overflow-y-auto no-scrollbar">
                  <ErrorBanner error={error} />
                  <form onSubmit={handleVendorLogin} className="space-y-4">
                        ...
                        {["food", "store"].map((type) => (
                          <button
                            ...
                            className={`flex-1 py-2.5 text-[13px] font-bold rounded-xl capitalize transition-all ${
                              loginType === type 
                                ? "bg-white dark:bg-[#0D0D17] shadow-sm text-gray-900 dark:text-gray-100 border border-gray-200 dark:border-[#2A2A3A]/50" 
                    ...
                    <div className="pt-2">
                      <BtnPrimary t={t} loading={loading}>Log In as Vendor</BtnPrimary>
                    </div>
                  </form>
                </div>
              </>
            ...
            {flow === "manager-login" && (
              <>
                <ModalHeader title="Manager Login" back="choose" reset={reset} t={t} onClose={onClose} />
                <div className="p-6 sm:p-8 relative z-10 overflow-y-auto no-scrollbar">
                  <ErrorBanner error={error} />
                  <form onSubmit={handleManagerLogin} className="space-y-4">
                    ...
                    <div className="pt-2">
                      <BtnPrimary t={t} loading={loading}>Log In as Manager</BtnPrimary>
                    </div>
                  </form>
                </div>
              </>
            ...
            {flow === "success" && (
              <>
                ...
                <div className="p-6 sm:p-10 flex flex-col items-center text-center overflow-y-auto no-scrollbar relative z-10">
                  ...
                  <h3 className="text-2xl font-black text-gray-900 dark:text-gray-100 mb-2">Success!</h3>
                  <p className="text-[15px] font-medium text-gray-500 dark:text-gray-400 mb-8 max-w-[250px] mx-auto leading-relaxed">{successMsg}</p>
