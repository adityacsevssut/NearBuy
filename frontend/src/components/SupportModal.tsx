...
import { X, Send, CheckCircle, Smartphone, MessageCircle } from "lucide-react";
...
export default function SupportModal({ isOpen, onClose, type }: SupportModalProps) {
  ...
  const [contactMethod, setContactMethod] = useState<"mobile" | "whatsapp">("mobile");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API}/api/public/support`, {
        ...
        body: JSON.stringify({ email, issue, contact_method: contactMethod, contact_number: contactNumber, type }),
      });
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          >
            ...
            <div className="px-6 py-5 border-b border-gray-100 dark:border-[#2A2A3A] flex justify-between items-center bg-gray-50/50 dark:bg-[#151522]/50">
              <div>
                <h3 className="font-black text-xl text-gray-900 dark:text-white tracking-tight">Need Support</h3>
                <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mt-0.5">We are here to help you</p>
              </div>
              <button
                onClick={handleClose}
            ...
            <div className="p-6">
              {!isLoggedIn ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">You must be logged in to request support.</p>
                ...
                >
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Our team will get back to you shortly.</p>
                </motion.div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">Describe your issue</label>
                    <textarea
                      required
                      rows={4}
                      value={issue}
                      ...
                      className={`w-full px-4 py-3 bg-gray-50 dark:bg-[#151522] border border-gray-200 dark:border-[#2A2A3A] ${theme.focus} rounded-xl text-sm outline-none transition-all resize-none dark:text-white`}
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-2">How can we reach you?</label>
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      <button
                        type="button"
                        onClick={() => setContactMethod("mobile")}
                        className={`flex items-center justify-center gap-2 py-3 rounded-xl border-2 transition-all ${contactMethod === "mobile" ? `${theme.activeTab} font-bold` : "border-gray-200 dark:border-[#2A2A3A] text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-[#151522]"}`}
                      >
                      ...
                      <button
                        ...
                        className={`flex items-center justify-center gap-2 py-3 rounded-xl border-2 transition-all ${contactMethod === "whatsapp" ? "border-green-500 bg-green-50 dark:bg-green-500/10 text-green-600 dark:text-green-400 font-bold" : "border-gray-200 dark:border-[#2A2A3A] text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-[#151522]"}`}
                      >
                    ...
                    <input
                      ...
                      placeholder={contactMethod === "mobile" ? "Enter Mobile Number" : "Enter Whatsapp Number"}
