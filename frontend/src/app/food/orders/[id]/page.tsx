      ...
      const options = {
        ...
        handler: async function (response: any) {
          try {
            ...
            toast.error(err.message || "Advance verification failed");
          } finally {
            setIsPaying(false);
          }
        },
      ...
      const options = {
        ...
        handler: async function (response: any) {
          try {
            ...
            toast.error(err.message || "Final payment verification failed");
          } finally {
            setIsPaying(false);
          }
        },
  ...
  const generateReceipt = async () => {
    ...
    if (element) {
      try {
        ...
        pdf.save(`Receipt_ZyphCart_${id}.pdf`);
      } catch (err) {
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    {order.refund_ticket.status === 'Awaiting UPI' && (
                      <div className="mt-4 p-4 bg-orange-50 dark:bg-orange-500/10 border border-orange-200 dark:border-orange-500/20 rounded-xl flex flex-col gap-3">
                        <div>
                          <p className="text-xs font-bold text-orange-gradient dark:text-orange-gradient mb-1">Action Required</p>
            ...
            <div className="flex gap-3">
              ...
              <div>
                <p className="font-black text-gray-900 dark:text-gray-100 leading-tight">
                  {order.delivery_address?.fullAddress || order.delivery_address?.landmark || order.delivery_address?.locationName || 'Saved Location'}
                </p>
        ...
        <div className="grid grid-cols-2 gap-3 mb-8">
          ...
          {currentStepIndex >= 3 && !isCancelled && order.delivery_boy_number && (
            <a 
              ...
              className={`w-full py-4 bg-orange-50 hover:bg-orange-100 border border-orange-200 rounded-2xl flex items-center justify-center gap-2 text-orange-600 font-bold text-sm transition-colors shadow-sm ${(showPayNow || showAdvancePayNow) ? 'col-span-1' : 'col-span-2 sm:col-span-1'}`}
            >
              <Phone className="w-4 h-4" />
              Call Delivery Boy
            </a>
