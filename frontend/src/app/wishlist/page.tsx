    ...
    <div className="min-h-screen bg-gray-50 flex flex-col pt-16 pb-24 md:pb-8">
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 sm:px-6 py-6">
        <div className="flex items-center gap-3 mb-6">
          <Link href="/" className="p-2 bg-white border border-gray-200 rounded-full text-gray-500 hover:text-orange-500 shadow-sm transition-colors">
        ...
        {restaurantWishlist.length === 0 && foodWishlist.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-gray-400 bg-white rounded-3xl border border-gray-200 shadow-sm mt-8">
            <Heart className="w-16 h-16 mb-4 text-gray-300" />
            <p className="font-bold text-gray-600 text-lg">Your wishlist is empty</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {restaurantWishlist.map((r) => (
                <Link
                  href={`/vendor/${r.id}`}
                  key={r.id}
                  className={`relative group flex flex-col bg-white rounded-2xl border border-gray-200 hover:border-orange-500 hover:shadow-lg hover:shadow-orange-500/10 text-left transition-all duration-300 overflow-hidden ${r.isClosed ? "opacity-75" : ""}`}
                >
                  <div className="relative w-full h-40 bg-gray-100 flex items-center justify-center border-b border-gray-100 overflow-hidden">
                    {r.image_url ? (
                    ...
                    {r.isClosed && (
                      <div className="absolute inset-0 bg-white/40 flex items-center justify-center z-20 pointer-events-none">
                        <span className="text-red-600 font-black text-xs uppercase tracking-widest px-3 py-1 bg-white/80 rounded-md shadow-sm">
                          Closed Now
                        </span>
                      </div>
                    )}
                  ...
                  <div className="p-3.5 flex flex-col justify-between flex-1 w-full bg-white">
                    <div>
                      <div className="flex justify-between items-start mb-0.5">
                        <p className="font-black text-gray-900 text-base tracking-tight truncate">{r.name}</p>
                      ...
                      <p className="text-xs text-gray-500 mb-2 truncate">{r.type}</p>
          ...
          <div>
            ...
            <div className="space-y-4">
              {foodWishlist.map((dish) => (
                <div
                  key={dish.id}
                  className={`relative bg-white p-4 rounded-2xl border border-gray-200 shadow-sm transition-all duration-300 flex gap-4 ${dish.is_available === false ? "opacity-75 grayscale-[0.2]" : "hover:border-orange-300 hover:shadow-md"}`}
                >
                  ...
                  <div className="relative flex flex-col items-center justify-start w-32 flex-shrink-0 mb-10">
                    <div className="w-32 h-32 bg-gray-100 rounded-xl border border-gray-200 overflow-hidden shadow-sm relative flex items-center justify-center">
                      ...
                      <button
                        ...
                        className="absolute top-2 right-2 p-1.5 rounded-full bg-white/80 backdrop-blur-sm border border-gray-200 shadow-sm hover:scale-110 transition-transform z-20"
                      >
