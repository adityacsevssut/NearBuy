    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0D0D17] flex flex-col pt-16 pb-24 md:pb-8">
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 sm:px-6 py-6">
        <div className="flex items-center gap-3 mb-6">
          <Link href="/" className="p-2 bg-white dark:bg-[#0D0D17] border border-gray-200 dark:border-[#2A2A3A] rounded-full text-gray-500 dark:text-gray-400 hover:text-emerald-500 shadow-sm transition-colors">
        ...
        {restaurantWishlist.length === 0 && foodWishlist.length === 0 && (
          <div className="flex flex-col items-center justify-center py-32 sm:py-40 text-gray-400 bg-white dark:bg-[#0D0D17] rounded-3xl border border-gray-200 dark:border-[#2A2A3A] shadow-sm mt-8">
            ...
            <Link href="/" className="mt-6 px-6 py-2.5 bg-emerald-500 text-white font-bold rounded-xl shadow-sm shadow-emerald-500/20 hover:bg-emerald-600 transition-colors">
              Explore Food
            </Link>
          </div>
        )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {restaurantWishlist.map((r) => (
                <Link
                  href={`/vendor/${r.id}`}
                  key={r.id}
                  className={`relative group flex flex-col bg-white dark:bg-[#0D0D17] rounded-2xl border border-gray-200 dark:border-[#2A2A3A] hover:border-emerald-500 hover:shadow-lg hover:shadow-emerald-500/10 text-left transition-all duration-300 overflow-hidden ${r.isClosed ? "opacity-75" : ""}`}
                >
                  <div className="relative w-full h-40 bg-gray-100 dark:bg-[#1F1F2E] flex items-center justify-center border-b border-gray-100 dark:border-[#2A2A3A] overflow-hidden">
                    {r.image_url ? (
                      ...
                      <div className="w-full h-full bg-emerald-50 flex items-center justify-center">
                        <Utensils className="w-10 h-10 text-emerald-200" />
                      </div>
                    )}
                  ...
                  <div className="p-3.5 flex flex-col justify-between flex-1 w-full bg-white dark:bg-[#0D0D17]">
                    <div>
                      <div className="flex justify-between items-start mb-0.5">
                        ...
                        <span className="flex items-center gap-1 text-[10px] font-bold text-white bg-emerald-600 px-1.5 py-0.5 rounded shadow-sm">
                      ...
                      <div className="flex items-center gap-2 text-[10px] font-medium text-gray-500 dark:text-gray-400">
                        {r.distance && (
                          <span className="bg-emerald-50 border border-emerald-100 px-2 py-1 rounded-md shadow-sm text-emerald-600 font-bold flex items-center gap-0.5">
                            📍 {r.distance}
                          </span>
                        )}
                      </div>
        ...
        {foodWishlist.length > 0 && (
          <div>
            <h2 className="font-black text-xl text-gray-800 dark:text-gray-200 mb-4 flex items-center gap-2">
              <Utensils className="w-5 h-5 text-emerald-500" /> Saved Dishes
            </h2>
            <div className="space-y-4">
              {foodWishlist.map((dish) => (
                <div
                  key={dish.id}
                  className={`relative bg-white dark:bg-[#0D0D17] p-4 rounded-2xl border border-gray-200 dark:border-[#2A2A3A] shadow-sm transition-all duration-300 flex gap-4 ${dish.is_available === false ? "opacity-75 grayscale-[0.2]" : "hover:border-emerald-300 hover:shadow-md"}`}
                >
                  <div className="flex-1 min-w-0 flex flex-col justify-between">
                    <div>
                      ...
                      <Link href={`/vendor/${dish.restaurantId}`} className="text-xs font-semibold text-gray-400 hover:text-emerald-500 transition-colors mb-1 block">
                        ...
                        <div className="w-full py-1.5 border border-red-200 font-black text-[10px] rounded-lg shadow-sm bg-red-50 text-red-600 text-center uppercase tracking-wider">
                          ...
                          <button 
                            ...
                            className={`w-full py-1.5 border font-black text-xs rounded-lg shadow-sm hover:shadow transition-all flex items-center justify-center uppercase tracking-wide ${
                              ...
                              : "bg-white dark:bg-[#0D0D17] text-emerald-600 border-gray-200 dark:border-[#2A2A3A] hover:bg-emerald-50"
                            }`}
                          >
